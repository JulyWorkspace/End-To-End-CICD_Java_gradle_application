pipeline{
    ...
    stages{
     ...
     post {
         always {
			mail bcc: '', body: ${env.JOB_NAME} <br>Build Number: ${env.BUILD_NUMBER} <br> URL de build: ${env.BUILD_URL} <br>Status: ${currentBuild.result}</b>", cc: '', charset: 'UTF-8', from: '', mimeType: 'text/html', replyTo: '', subject: "${currentBuild.result} CI: Project name -> ${env.JOB_NAME}", to: "kunalumrane@gmail.com";  
		 }
	   }
}
// pipeline{
